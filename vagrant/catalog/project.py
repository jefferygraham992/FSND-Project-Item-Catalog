...
def catalogJSON():
    ...
    return jsonify(catalog=[train_kind.serialize for train_kind in inventory])


@app.route('/catalog/<train_type>/JSON')
def trainsJSON(train_type):
    items = session.query(Character).filter_by(character_kind=train_type).all()
    return jsonify(engines=[item.serialize for item in items])
...
def showTrains(train_type):
    inventory = session.query(CharacterType).all()
    trainType = (
                 session.query(CharacterType)
                 .filter_by(type_name=train_type)
                 .one()
                 )
    trains = (
              session.query(Character)
              .filter_by(character_kind=train_type)
              .all()
              )
    if 'username' not in login_session:
        return render_template('publicshowtrains.html',
                               trainType=trainType,
                               trains=trains,
                               inventory=inventory)
    else:
        return render_template('showtrains.html',
                               trainType=trainType,
                               trains=trains,
                               inventory=inventory)
...
def showTrain(train_type, train_name):
    ...
    if (
        ...
        return render_template('publicshowtrain.html',
                               train=train,
                               creator=creator)
    else:
        return render_template('showtrain.html',
                               train=train,
                               creator=creator)
...
def addTrain():
    ...
    if request.method == 'POST':
        newTrain = Character(character_name=request.form['character_name'],
                             description=request.form['description'],
                             character_kind=request.form['character_kind'],
                             character_picture=request.form
                             ['character_picture'],
...
def editTrain(train_name):
    ...
    editedTrain = (
                   session.query(Character)
                   .filter_by(character_name=train_name)
                   .one()
                   )
    if editedTrain.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not authorized"
        " to edit this train. Please create your own train "
    ...
    if request.method == 'POST':
        ...
        return render_template('edittrain.html',
                               editedTrain=editedTrain,
                               inventory=inventory)
...
def deleteTrain(train_name):
    ...
    if trainToDelete.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('You are not authorized "
        "to delete this train. Please create your own train in order "
